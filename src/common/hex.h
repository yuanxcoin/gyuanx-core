...
namespace tools {
  ...
  bool hex_to_type(std::string_view hex, T& x) {
    if (!gyuanxmq::is_hex(hex) || hex.size() != 2*sizeof(T))
      return false;
    gyuanxmq::from_hex(hex.begin(), hex.end(), reinterpret_cast<char*>(&x));
    return true;
  }
  ...
  std::string type_to_hex(const T& val) {
    return gyuanxmq::to_hex(std::string_view{reinterpret_cast<const char*>(&val), sizeof(val)});
  }
}
