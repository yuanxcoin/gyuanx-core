...
namespace gnodes {
  ...
  constexpr size_t pulse_min_gnodes(cryptonote::network_type nettype)
  {
    return (nettype == cryptonote::MAINNET) ? 50 : PULSE_QUORUM_SIZE;
  }
  static_assert(pulse_min_gnodes(cryptonote::MAINNET) >= PULSE_QUORUM_SIZE);
  static_assert(pulse_min_gnodes(cryptonote::TESTNET) >= PULSE_QUORUM_SIZE);
  ...
  //
  // The default, starting in Gyuanx 8, subtracts two blocks for every block you were decomissioned,
  ...
  // credit.
  //
  // Before Gyuanx 8 (when this configuration was added) recomissioning would always reset your credit
...
#ifndef GYUANX_ENABLE_INTEGRATION_TEST_HOOKS
  ...
  constexpr std::array<int, 3> MIN_GYUANXNET_VERSION{{0, 8, 0}};
...
// return: UINT64_MAX if (num_contributions > the max number of contributions), otherwise the amount in gyuanx atomic units
...
bool check_gnode_portions(uint8_t version, const std::vector<uint64_t>& portions);
