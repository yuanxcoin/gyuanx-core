...
gsigs = 'https://github.com/loki-project/gitian.sigs.git'
...
def setup():
    ...
    if not os.path.isdir('loki'):
        subprocess.check_call(['git', 'clone', args.url, 'loki'])
    os.chdir('..')
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/loki/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])

    if args.linux:
        print('\nCompiling ' + args.version + ' Linux')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'loki='+args.commit, '--url', 'loki='+args.url, 'inputs/loki/contrib/gitian/gitian-linux.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-linux', '--destination', '../sigs/', 'inputs/loki/contrib/gitian/gitian-linux.yml'])
        subprocess.check_call('mv build/out/loki-*.tar.bz2 ../out/'+args.version, shell=True)

    if args.windows:
        print('\nCompiling ' + args.version + ' Windows')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'loki='+args.commit, '--url', 'loki='+args.url, 'inputs/loki/contrib/gitian/gitian-win.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-win', '--destination', '../sigs/', 'inputs/loki/contrib/gitian/gitian-win.yml'])
        subprocess.check_call('mv build/out/loki*.zip ../out/'+args.version, shell=True)

    if args.macos:
        print('\nCompiling ' + args.version + ' MacOS')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'loki='+args.commit, '--url', 'loki'+args.url, 'inputs/loki/contrib/gitian/gitian-osx.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx', '--destination', '../sigs/', 'inputs/loki/contrib/gitian/gitian-osx.yml'])
        subprocess.check_call('mv build/out/loki*.tar.bz2 ../out/'+args.version, shell=True)
...
def verify():
    ...
    subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-linux', 'inputs/loki/contrib/gitian/gitian-linux.yml'])
    print('\nVerifying v'+args.version+' Windows\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-win', 'inputs/loki/contrib/gitian/gitian-win.yml'])
    print('\nVerifying v'+args.version+' MacOS\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-osx', 'inputs/loki/contrib/gitian/gitian-osx.yml'])
    os.chdir(workdir)
...
def main():
    ...
    os.makedirs('builder/inputs/loki', exist_ok=True)
    os.chdir('builder/inputs/loki')
    if args.pull:
