...
RUN set -ex && \
    ldd /src/build/release/bin/gyuanxd
...
# Create gyuanx user
RUN adduser --system --group --disabled-password gyuanx && \
	mkdir -p /wallet /home/gyuanx/.gyuanx && \
	chown -R gyuanx:gyuanx /home/gyuanx/.gyuanx && \
	chown -R gyuanx:gyuanx /wallet

# Contains the blockchain
VOLUME /home/gyuanx/.gyuanx
...
ENTRYPOINT ["gyuanxd", "--p2p-bind-ip=0.0.0.0", "--p2p-bind-port=22022", "--rpc-bind-ip=0.0.0.0", "--rpc-bind-port=22023", "--non-interactive", "--confirm-external-bind"]
