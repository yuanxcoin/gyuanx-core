    ...
    steps: [
        submodules,
        {
            ...
            commands: [
                ...
                if test_oxend then [
                    '(sleep 3; echo "status\ndiff\nexit") | TERM=xterm ../utils/build_scripts/drone-gdb.sh ./bin/gyuanxd --offline --data-dir=startuptest'
                ] else []
            ) + (
                if run_tests then [
                    'mkdir -v -p $$HOME/.oxen',
...
local gui_wallet_step(image, wine=false) = {
    ...
    commands: [
        ...
        'git clone https://github.com/yuanxcoin/gyuanx-electron-gui-wallet.git',
        'cp -v build/bin/gyuanxd' + (if wine then '.exe' else '') + ' gyuanx-electron-gui-wallet/bin',
        'cp -v build/bin/gyuanx-wallet-rpc' + (if wine then '.exe' else '') + ' gyuanx-electron-gui-wallet/bin',
...
local gui_wallet_step_darwin = {
    ...
    commands: [
        'git clone https://github.com/yuanxcoin/gyuanx-electron-gui-wallet.git',
        'cp -v build/bin/{gyuanxd,gyuanx-wallet-rpc} gyuanx-electron-gui-wallet/bin',
        'cd gyuanx-electron-gui-wallet',
        'sed -i -e \'s/^\\\\( *"version": ".*\\\\)",/\\\\1-${DRONE_COMMIT_SHA:0:8}",/\' package.json',
        'npm install',
        'npm run build',
...
[
    ...
    debian_pipeline("Static (win64)", "ubuntu:focal", deps='g++ g++-mingw-w64-x86-64 '+static_build_deps,
                    ...
                    build_tests=false, lto=false, test_gyuanxd=false, extra_cmds=[
